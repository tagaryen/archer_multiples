...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body {
    ...
    align-items: center;
    background: linear-gradient(135deg, #ced5f8ff 0%, #d5b8f1ff 100%);
...
<div class="player">
  ...
  <div class="video-wrap" id="videoWrap">
    ...
    <div class="play-overlay" id="overlay"></div>
  </div>
  <div class="controls">
...
function handleVideoHtml(title, icon, contentType, videoName) {
    let html = videoHtml;
    html = html.replace("$(title_template)", title);
    html = html.replace("$(icon_template)", icon);
    html = html.replace("$(video_name)", videoName);
    html = html.replace("$(video_type)", contentType);
    return html.replace("$(video_src)", "/video/"+videoName);
}

module.exports = handleVideoHtml;
